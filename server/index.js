...
if (req.method === 'POST' && pathname === '/api/uploads') {
  try {
    ...
    if (!boundaryMatch) {
      sendJson(res, 400, { error: 'Invalid form data (missing boundary)' });
      return;
    }
    ...
    req.on('end', () => {
      ...
      if (!filePart) {
        sendJson(res, 400, { error: 'File not found in form data' });
        return;
      }

      const match = filePart.match(/filename="(.+?)"/);
      const fileName = match ? match[1] : 'uploaded.pdf';

      // Extract the binary content between headers and boundary
      ...
      const previewPath = path.join(UPLOAD_DIR, `${uploadId}.preview.txt`);
      fs.writeFileSync(previewPath, `Uploaded file: ${fileName}`);
